    ...
    for linha in linhas_divididas:
        ...
        match linha[0]:
            case "0010011" if (linha[2] == "001" or linha[2] == "101"):
                ...
                name = createName(linha[0], str(int(linha[2], 2))+str(int(not (linha[linha[-1][1]] == "1"))))
                instrucoes.append(f"Formato = I {name} | rd = x{rd_int} | f3 = {func3_int} | rs1 = x{rs1_int} | shamt = {shamt}")
                estrutura_instrucoes.append((op_code, rd_int, func3_int, rs1_int, rs2_int))
            case "0010011":
                immed = bin_to_int(resto, signed=True)
                name = createName(linha[0]+"-A", linha[2])
                instrucoes.append(f"Formato = I {name} | rd = x{rd_int} | f3 = {func3_int} | rs1 = x{rs1_int} | imed = {immed}")
                estrutura_instrucoes.append((op_code, rd_int, func3_int, rs1_int, immed))
            case "0000011":
                immed = bin_to_int(resto, signed=True)
                name = createName(linha[0], linha[2])
                instrucoes.append(f"Formato = I {name} | rd = x{rd_int} | f3 = {func3_int} | rs1 = x{rs1_int} | imed = {immed}")
                estrutura_instrucoes.append((op_code, rd_int, func3_int, rs1_int, immed))
            case "1110011":
            ...
            case "0110011":
                ...
                name = createName(linha[0], str(int(linha[2], 2))+str(int(not (func7 == "0100000"))))
                instrucoes.append(f"Formato = R {name} | rd = x{rd_int} | f3 = {func3_int} | rs1 = x{rs1_int} | rs2 = x{rs2_int}")
                estrutura_instrucoes.append((op_code, rd_int, func3_int, rs1_int, rs2_int))
            case "0110111":
            ...
            case "0100011":
                ...
                name = createName(linha[0], linha[2])
                instrucoes.append(f"Formato = S {name} | rd = x{rd_int} | f3 = {func3_int} | rs1 = x{rs1_int} | rs2 = x{rs2_int} | imed = {immed}")
                estrutura_instrucoes.append((op_code, rd_int, func3_int, rs1_int, rs2_int))
            case "1100011": # branches
                ...
                name = createName(linha[0], linha[2])
                instrucoes.append(f"Formato = B {name} | rs1 = x{rs1_int} | rs2 = x{rs2_int} | f3 = {func3_int} | imed = {immed}")
